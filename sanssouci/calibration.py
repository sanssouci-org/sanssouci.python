...
def get_perm_p(X, categ, B=100 , row_test_fun=scipy.stats.ttest_ind):
  """
  ...
  - row_test_fun: testing function with the same I/O as 'scipy.stats.ttest_ind' (default). 
                ...
                Eligible functions are for instance "scipy.stats.ks_2samp", 
                "scipy.stats.bartlett", "scipy.stats.ranksums", "scipy.stats.kruskal"
  ...
  if row_test_fun==tst.row_welch_tests: #row Welch Tests (parallelized)
    for bb in range(B):
      swt=tst.row_welch_tests(X, shuffled_categ_all[bb,:])
    ...
    for bb in range(B):
      ...
      for ii in range(p):
        rwt=row_test_fun(X[s0, ii], X[s1, ii])  #Welch test with scipy -> rwt=scipy.stats.ttest_ind(X[s0, ii], X[s1, ii],equal_var=False)
...
def get_pivotal_stats(p0,  t_inv=fam.t_inv_linear, K = -1):
  """
