...
def curve_max_fp(p_values, thr):
  ...
  * Reference:
  ...
  K =  np.ones(p)*(kMax-1)  ## K[i] = number of k/ T[i] <= s[k] = BB in 'Mein2006'
  Z = np.ones(kMax)*(p-1)  ## Z[k] = number of i/ T[i] >  s[k] = cardinal of R_k
  ...
  cA = cummax_A[K_ww]  # cA[i] = max_{k<K[i]} A[k]
  
  Vbar[ww] = np.min(np.concatenate(( (ww-cA).reshape(1,-1) , (K[ww]).reshape(1,-1) ),axis=0),axis=0)
